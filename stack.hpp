...
template <typename T> class stack {
  ...
  public:
    ...
    ~stack() {
      while(head != NULL) {  // while the head node is not NULL 
        node *temp = head;
        head = head->next;
    ...
    node *look_for_memory(size_t size){
      node *temp = start;
      while(temp != NULL) {  // while the start node is not NULL
        if(temp->is_free && temp->size >= size) // checking the status of the node (if it is free) and the size of the node (if it is greater than or equal to the size we need)
          return temp;
        temp = temp->next; 
      }
        return (NULL);
    ...
    node *update_node(node *nnode, size_t size, void *memory_block){
      ...
      void * my_malloc(unsigned long size){
        ...
        if(memory_block == (void*) - 1) { // if the allocation fails
          pthread_mutex_unlock(&lock);
          return (NULL);
        }
        new_node = update_node(new_node, size, memory_block); // update the node using the update_node function
...

{
    ...
    new_brk = sbrk(0);  // get the current sbrk value

    if ((char*)ptr + new_node->size == new_brk) {   // if the node is the last node

        if (start == end1) { // if the start and end nodes are the same we assign the start and end nodes to NULL
